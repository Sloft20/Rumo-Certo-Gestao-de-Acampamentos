...
import { RefreshCw, AlertCircle, Paperclip, X } from 'lucide-react';
import CustomSelect from './CustomSelect'; 
import CurrencyInput from './CurrencyInput'; 

export default function TransactionForm({
  guardarRegistro, tipo, setTipo,
  ...
  const opcoesPagamento = [{ value: 'PIX', label: 'Pix' }, { value: 'DINHEIRO', label: 'Dinheiro Físico' }, { value: 'CARTÃO', label: 'Cartão' }];
  ...
  return (
    <div className="animate-in fade-in duration-300">
      ...
      {/* Removemos as bordas e o fundo, pois a Gaveta já faz isso */}
      <form onSubmit={guardarRegistro} className="flex flex-col gap-5 pt-2 pb-8">
        ...
        {isInscricao ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <CurrencyInput label="Total Combinado" value={valorTotal} onChange={e => setValorTotal(e.target.value)} required={true} placeholder="Ex: R$ 180,00" />
            <CurrencyInput label="Recebido Agora" value={valorPago} onChange={e => setValorPago(e.target.value)} required={true} placeholder="Ex: R$ 50,00" />
          </div>
        ) : (
          <CurrencyInput label="Valor da Operação" value={valorPago} onChange={e => setValorPago(e.target.value)} required={true} placeholder="Ex: R$ 150,00" />
        )}
        ...
        <div>
          ...
          <textarea value={observacao} onChange={e => setObservacao(e.target.value)} className={`${inputClass} resize-y min-h-[100px]`} placeholder="Algum detalhe importante?" />
        </div>
